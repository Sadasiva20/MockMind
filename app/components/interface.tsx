...
export default function InterviewInterface() {
  ...
  async function loadProblem() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(body?.error || "Unable to load a problem from the backend.");
      }
      setProblem(body);
    } catch (err) {
  ...
  async function callAgent(command: string, payload: Record<string, unknown>) {
    ...
    try {
      const response = await fetch("/api/agent", {
        ...
        body: JSON.stringify({ command, userId: USER_ID, ...payload }),
      });
      ...
      setAgentError(err instanceof Error ? err.message : "Agent call failed.");
      return null;
    } finally {
  ...
  async function requestAgentReview(command: "review_progress" | "recommend_problem") {
    ...
    if (command === "review_progress") payload.problemId = problem?.id ?? null;
    ...
    <div className="grid gap-8 lg:grid-cols-[320px_1fr]">
      <section className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
        <div className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
              Switch between behavioral interview practice, system design, and the new LeetCode-style challenge mode.
            </p>
          </div>
          ...
          <div className="grid gap-2">
            {modes.map((item) => (
              ...
              >
                ...
                {item === "LeetCode Mode" ? (
                  <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">New</div>
                ) : null}
              </button>
            ))}
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm font-semibold text-slate-900 dark:text-white">Agent coaching</p>
          <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <div>Use the AI coach to review progress, recommend your next problem, or get a study plan.</div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
          <p className="text-sm uppercase tracking-[0.24em] text-sky-600">Progress</p>
          <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <div>Attempts: {attempts}</div>
            <div>Last results:</div>
            <ul className="list-disc space-y-1 pl-5">
              {resultHistory.length > 0 ? (
                resultHistory.map((entry) => <li key={entry}>{entry}</li>)
              ) : (
        ...
        {mode !== "LeetCode Mode" ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
            ...
            <p className="mt-3 text-slate-600 dark:text-slate-300">
              {mode === "Mock Interview"
                ? "Practice behavioral questions with AI coaching and conversation guidance."
                : "Practice system design walkthroughs with architecture prompts and scalability feedback."}
            </p>
            <div className="mt-8 rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
              This interface is being extended to support the full behavioral and system design flows. For now, switch to LeetCode Mode for coding-focused practice.
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-sky-600">Problem</p>
                  <h2 className="mt-1 text-3xl font-semibold">{problem?.title ?? "Loading problem..."}</h2>
                  {problem ? (
              ...
              <div className="mt-6 space-y-5 text-sm leading-7 text-slate-700 dark:text-slate-300">
                {problem ? (
                  ...
                  <p className="text-slate-600 dark:text-slate-400">Loading a problem from the backend...</p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Problem filters</p>
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                    Pick a topic or difficulty and load a matching problem from MongoDB.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="space-y-2 text-sm text-slate-700 dark:text-slate-300">
                  ...
                  <select
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-200 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-sky-400 dark:focus:ring-slate-700"
                  >
                    {topics.map((topic) => (
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Your solution</p>
                  <h3 className="mt-2 text-xl font-semibold">Explain your approach</h3>
                </div>
              ...
              <textarea
                ...
                placeholder="Describe your solution, algorithm, and edge-case handling here..."
                rows={11}
                className="mt-5 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-6 text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-200 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-sky-400 dark:focus:ring-slate-700"
              />
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-slate-600 dark:text-slate-400">Share your full approach before submitting for AI review.</p>
                <button
                  type="button"
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-sky-600">Feedback</p>
                  <h3 className="mt-2 text-2xl font-semibold">AI evaluation</h3>
                </div>
                {feedback ? (
                  <span className={`rounded-2xl px-3 py-1 text-sm font-semibold ${feedback.correctness === "correct" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300" : "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300"}`}>
                    {feedback.correctness === "correct" ? "Looks good" : "Needs improvement"}
                  </span>
                ) : (
                  <span className="rounded-2xl bg-slate-100 px-3 py-1 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-300">Awaiting submission</span>
                )}
              </div>
              ...
              <div className="mt-5 space-y-5 text-sm leading-7 text-slate-700 dark:text-slate-300">
                <div className="rounded-3xl bg-slate-50 p-5 dark:bg-slate-900">
                  ...
                  <p className="mt-2">{feedback ? feedback.confidence : "Submit your answer to see correctness and next steps."}</p>
                </div>
                <div className="grid gap-4 lg:grid-cols-3">
                  <div className="rounded-3xl bg-slate-50 p-5 dark:bg-slate-900">
                    <p className="font-semibold">Time complexity</p>
                    <p className="mt-2">{feedback ? feedback.timeComplexity : "Waiting for AI review."}</p>
                  </div>
                  <div className="rounded-3xl bg-slate-50 p-5 dark:bg-slate-900">
                    <p className="font-semibold">Hint</p>
                    <p className="mt-2">{feedback ? feedback.hint : "The evaluation will recommend a hint here."}</p>
                  </div>
                  <div className="rounded-3xl bg-slate-50 p-5 dark:bg-slate-900">
                    <p className="font-semibold">Follow-up</p>
                    <p className="mt-2">{feedback ? feedback.followUp : "A follow-up question will appear after review."}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-sky-600">Coach summary</p>
                  <h3 className="mt-2 text-2xl font-semibold">Agent recommendations</h3>
                </div>
                {agentLoading ? (
                  <span className="rounded-2xl bg-slate-100 px-3 py-1 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-300">Analyzing...</span>
                ) : (
                  <span className="rounded-2xl bg-slate-100 px-3 py-1 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-300">Coach powered by Gemini + MongoDB</span>
                )}
              </div>
              ...
              <div className="mt-5 space-y-5 text-sm leading-7 text-slate-700 dark:text-slate-300">
                <div className="rounded-3xl bg-white p-5 dark:bg-slate-900">
                  ...
                  <p className="mt-2">{agentAdvice ?? "Submit a solution or ask the coach to review your progress."}</p>
                </div>
                <div className="rounded-3xl bg-white p-5 dark:bg-slate-900">
                  <p className="font-semibold">Latest progress note</p>
                  <p className="mt-2">{progressSummary ?? "No coach summary available yet."}</p>
                </div>
                <div className="rounded-3xl bg-white p-5 dark:bg-slate-900">
                  <p className="font-semibold">Next action</p>
                  <p className="mt-2">{nextAction ?? "Ask the coach for a recommendation after submitting your first answer."}</p>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[320px_1fr]">
      <section className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-zinc-950">
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm font-semibold text-slate-900 dark:text-white">Progress</p>
          <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <div>Attempts: {attempts}</div>
            <div>Last results:</div>
            <ul className="list-disc space-y-1 pl-5">
